...
use crate::{
    database::{
        leaderboard::messages::{DisableSubmission, InsertLeaderboardRows, InsertSubmission},
        schema::{LeaderboardRow, SubmissionRow},
    },
    leaderboard_api::models::LeaderboardSubmission,
    API,
};
...
pub async fn submit_leaderboard_entries(
    state: Data<API>,
    body: Json<LeaderboardSubmission>,
) -> impl Responder {
    if body.entries.len() != 200 {
        return HttpResponse::BadRequest().body(
            String::from("Invalid entries length <") + &body.entries.len().to_string() + ">",
        );
    }
    ...
    let rows: Vec<LeaderboardRow> = body
        .entries
        .iter()
        ...
        .send(
            InsertSubmission {
                sub: SubmissionRow {
                    ...
                    valid: true,
                    unix_time_stamp: body.unix_time_stamp,
                },
            },
            SUBMISSION,
        )
        .await
    {
        ...
        Ok(Err(err)) => {
            return HttpResponse::InternalServerError().body(format!(
                "Submission insert didn't go through, won't be queried: {}",
                err
            ))
        }
    ...
    match state
        .db
        .send(InsertLeaderboardRows { rows }, SUBMISSION)
        .await
    {
        Ok(Ok(_)) => {
        ...
        Ok(Err(err)) => {
                ...
                .await
            {
                ...
                Ok(Err(err)) => {
                    return HttpResponse::InternalServerError().body(format!(
                        "Could not disable submission. We're in a bad sate! Error: {}",
                        err
                    ))
                }
                _ => return HttpResponse::InternalServerError().body("Cannot process request"),
